...
function pickTheme(room) {
  // Language is a per-device choice, not a room setting, so the theme goes out
  // in both languages and each client renders its own.
  const i = Math.floor(Math.random() * THEMES.da.length);
  room.theme = { da: THEMES.da[i], en: THEMES.en[i] };
}
